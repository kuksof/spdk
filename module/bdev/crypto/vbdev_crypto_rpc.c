...
decrypt_wrapped_key_to_hex(const char *wrapped_b64, const char *kek_hex, char **out_hex)
{
	...
	*out_hex = NULL;
	...
	kek_len = strlen(kek_hex) / 2;

	/* For AES-256-GCM we expect 32 bytes KEK */
	if (kek_len != 32) {
		SPDK_ERRLOG("kek_hex must be 32 bytes (64 hex chars) for AES-256-GCM, got %zu bytes\n", kek_len);
		rc = -EINVAL;
		goto cleanup;
	}

	kek_bin = (uint8_t *)spdk_unhexlify(kek_hex);
	if (!kek_bin) {
